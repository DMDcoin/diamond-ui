...
import { getDecimalsBasedOnUnitLength } from "../../utils/common";
...
const ProposalStepSlider: React.FC<ProposalStepSliderProps> = ({ paramsRange, state, setState }) => {
    ...
    return (
        <div className={styles.sliderContainer}>
            <span>{BigNumber(state).dividedBy(10**getDecimalsBasedOnUnitLength(state)).toString()} DMD</span>
            <div>
                <span>{BigNumber(paramsRange[0]).dividedBy(10**getDecimalsBasedOnUnitLength(state)).toString()} DMD</span>
                <input
                    type="range"
                    min={paramsRange[0]}
                ...
                />
                <span>{BigNumber(paramsRange[paramsRange.length - 1]).dividedBy(10**getDecimalsBasedOnUnitLength(state)).toString()} DMD</span>
            </div>
        </div>
    );
}
